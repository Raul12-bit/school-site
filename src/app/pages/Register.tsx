...
export function Register() {
  ...
  return (
    <Page label={t.profileLabel} title={t.authRegister}>
      <form className="form-card auth-card" onSubmit={submit}>
        ...
        <button className="button primary">{t.authRegister}</button>
        {error && <p className="form-message error">{error}</p>}
        <Link className="plain-button" to="/auth/login">{t.authLoginLink}</Link>
      </form>
    </Page>
  );
}
